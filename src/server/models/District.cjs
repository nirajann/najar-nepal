...
router.get("/", async (req, res) => {
  try {
    ...
    const districts = await District.find(filter)
      .populate("mpLeaders", "leaderId name role party currentOffice photo")
      .populate("ministerLeaders", "leaderId name role party currentOffice photo")
      .populate("naLeaders", "leaderId name role party currentOffice photo")
      .sort({ name: 1 });
